...
function Map() {
  ...
  const handleKeyPress = (event) => {
    if (event.key === "Enter") {
      ...
      setResults(foundCounties); // Display results
      setHighlightedCounties(foundCounties); // Highlight found counties
    }
  };
    ...
    <div className="w-screen h-screen bg-white flex flex-col items-center justify-center overflow-hidden">
      ...
      <div className="my-2 text-lg">
        {results.length > 0 ? (
          ...
          searchTerm && (
            <p className="text-red-500">City not found in Florida.</p>
          )
        )}
      </div>
      ...
      <div className="w-4/5 h-4/5">
        ...
        >
          <Geographies geography={geoUrl}>
            {({ geographies }) =>
              geographies.map((geo) => {
                const countyName = geo.properties.NAME; // Assuming the JSON has county names under 'NAME'
                ...
                return (
                  <Geography
                    key={geo.rsmKey}
                    geography={geo}
                    fill={isHighlighted ? "#FF5733" : "#E5E5F7"} // Highlight selected counties
                    stroke="#FFFFFF"
                    tabIndex={-1}
                    style={{
                      default: { outline: "none" },
                      hover: { fill: isHighlighted ? "#FF5733" : "#A9A9A9" },
