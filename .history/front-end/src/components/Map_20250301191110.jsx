...
function Map() {
  ...
  const handleKeyPress = (event) => {
    if (event.key === "Enter") {
      ...
      if (foundCounty) {
        setSelectedCounties(
          (prevSelected) => new Set([...prevSelected, foundCounty])
        );
      } else {
  ...
  const toggleCountySelection = (countyName) => {
              ...
              geographies.map((geo) => {
                ...
                return (
                  <Geography
                    key={geo.rsmKey}
                    geography={geo}
                    fill={selectedCounties.has(countyName) ? "blue" : "#E5E5F7"}
                    stroke="#FFFFFF"
                    tabIndex={-1}
                    onClick={() => toggleCountySelection(countyName)}
                    style={{
