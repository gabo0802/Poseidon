...
function Map() {
  ...
  useEffect(() => {
    if (selectedCounty) {
      // Smooth transition to show the panel
      setPanelStyle({
      ...
      // Smooth transition to hide the panel
      setPanelStyle({
        transform: "translateX(100%)",
        opacity: 0,
        transition: "transform 0.3s ease-out, opacity 0.3s ease-out",
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Smooth county data panel with inline styles */}
        <div
          className="county-info-panel w-1/3 h-[80%] p-4 shadow-2xl overflow-auto backdrop-blur-lg rounded-xl"
          style={{
            background:
              "linear-gradient(to bottom, rgba(255, 82, 2, 0.85), rgba(255, 143, 92, 0.85))",
            ...panelStyle, // applying the transition style
          }}
        >
