  ...
  const handleKeyPress = (event) => {
    if (event.key === "Enter") {
      ...
      if (foundCounties.length > 0) {
        setSelectedCounties(new Set(foundCounties)); // Update selected counties
      } else {
        console.log("City not found in Florida.");
        setSelectedCounties(new Set()); // Reset selection if not found
      }
    }
  };
              ...
              geographies.map((geo) => {
                ...
                return (
                  <Geography
                    key={geo.rsmKey}
                    geography={geo}
                    fill={selectedCounties.has(countyName) ? "blue" : "#E5E5F7"} // Fill based on selection
                    stroke="#FFFFFF"
                    tabIndex={-1}
                    style={{
                      default: { outline: "none" },
                      hover: { outline: "none", fill: "lightblue" },
                      pressed: { outline: "none", fill: "red" },
                    }}
                  />
                );
              })
